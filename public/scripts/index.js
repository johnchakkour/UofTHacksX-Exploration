$(document).ready(function() {
  ...
  function defaultMapSetUp(jsonData) {
    ...
    googleMap.addTo(map);
    JSON.parse(sessionStorage.getItem("jsonData"))["pins"].forEach(addMarkers);
  }

  if (!mapInitialized) {
    defaultMapSetUp(JSON.parse(sessionStorage.getItem("jsonData")["pins"]));
    mapInitialized = true;
  }
  ...
  function addToData(latitude, longitude, name, discription) {
    const currentPins = JSON.parse(sessionStorage.getItem("jsonData")["pins"]);
    currentPins.push({
      "LAT": latitude,
      "LNG": longitude,
      "NAME": name,
